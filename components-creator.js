...
module.exports = {
    createMolecule(moleculeName) {
        if (Common.checkIfCurrentProjectIsFLProject()) {
            ...
            if (Config.getConfigurable().include_spec) {
                Common.readTmplAndWrite(`${Config.installationPath()}/templates/molecule/tmpl-name.molecule.spec.ts.tmpl`, `${moleculePath}/${moleculeName}.molecule.spec.ts`, [moleculeName, moleculeNameClass], 'typescript', [Config.toReplace, /\[TMPL_NAME\:CLASS_CASE\]/g]);
            }
            Common.readTmplAndWrite(`${Config.installationPath()}/templates/molecule/tmpl-name.molecule.tsx${(Config.getConfigurable().include_md_reader) ? '' : '[NO_MD_READER]'}.tmpl`, `${moleculePath}/${moleculeName}.molecule.tsx`, [moleculeName, moleculeNameClass], 'typescript', [Config.toReplace, /\[TMPL_NAME\:CLASS_CASE\]/g]);
    ...
    createPage(pageName) {
        if (Common.checkIfCurrentProjectIsFLProject()) {
            ...
            if (Config.getConfigurable().include_spec) {
                Common.readTmplAndWrite(`${Config.installationPath()}/templates/page/tmpl-name.page.spec.ts.tmpl`, `${pagePath}/${pageName}.page.spec.ts`, [pageName, pageNameClass], 'typescript', [Config.toReplace, /\[TMPL_NAME:CLASS_CASE\]/g]);
            }
